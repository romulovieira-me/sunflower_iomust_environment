...
If this code/environment was useful to you, consider making a donation in PayPal: https://www.paypal.com/donate/?business=LDEF5HT4K644L&no_recurring=0&currency_code=BRL
...
def Goodbye(address: str):
    ...
    for addr in modo:  # If the address bound to the socket contains the word "output"
        mod_list = addr.split("/")  # Divide the address
        search = mod_list[2]  # Search for the word in the specified list
        if del_idx == search:  # If the ID number is the same as the one that was disconnected
            modo.remove(addr)  # Remove the item from the list
...
if __name__ == "__main__":
    ...
    dispatcher.map("/mode/*", getModo)  # Map /mode/* messages and call gethHello function
